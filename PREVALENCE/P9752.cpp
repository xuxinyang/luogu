...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        // 转动两个拨轮
        for (int j = 1; j <= 9; j++) f[(a+j)%10][(b+j)%10][c][d][e]++;
        for (int j = 1; j <= 9; j++) f[a][(b+j)%10][(c+j)%10][d][e]++;
        for (int j = 1; j <= 9; j++) f[a][b][(c+j)%10][(d+j)%10][e]++;
        for (int j = 1; j <= 9; j++) f[a][b][c][(d+j)%10][(e+j)%10]++;
    }
    for (int i = 0; i <= 9; i++)
        for (int j = 0; j <= 9; j++)
            for (int k = 0; k <= 9; k++)
